...
def load_data():
    ...
    try:
        xlsx = pd.ExcelFile('PAINELLI VENDAS 25 (2) - lucas.xlsx')
    except:
        try:
            xlsx = pd.ExcelFile('/Users/lucasegidiodantas/Downloads/PAINELLI VENDAS 25 (2) - lucas.xlsx')
        except:
            st.error("Arquivo não encontrado. Por favor, coloque o arquivo Excel na mesma pasta do app.")
            return None

    data_sheets = [s for s in xlsx.sheet_names if not s.startswith('RES')]

    all_data = []
    for sheet in data_sheets:
        df = pd.read_excel(xlsx, sheet_name=sheet)
        df = df.loc[:, ~df.columns.str.contains('^Unnamed')]
        df['Periodo_Aba'] = sheet
        all_data.append(df)
